...
from fer_models.apvit_vit_layers import DropPath
...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        q, k, v = qkv[0], qkv[1], qkv[2]   # make torchscript happy (cannot
        # use tensor as tuple)

        attn = (q @ k.transpose(-2, -1)) * self.scale   # [B, head_num,
        # token_num, token_num]
...
class HeadFusionAttention(nn.Module):
    ...
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None,
        ...
        self.qkv = nn.ModuleList(
            [nn.Linear(dim//self.group_number, (dim//self.group_number)*3,
                       bias=qkv_bias) for _ in range(self.group_number)])
        self.attn_drop = nn.Dropout(attn_drop)
        self.proj = nn.Linear(dim, dim)
        self.proj_drop = nn.Dropout(proj_drop)
