...
from fer_models.apvit_classifiers_base import BaseClassifier
from fer_models.irse import IR_50
from fer_models.apvit_vit_vit_siam_merge import PoolingViT
from fer_models.apvit_heads_linear_head import LinearClsHead
from fer_models.apvit_heads_linear_head import MultiLinearClsHead
...
def build_head(num_classes: int, dense_dims: str = '', in_channels: int = 768):
    assert isinstance(dense_dims, str), type(dense_dims)
    if dense_dims in ['', 'None']:
        return LinearClsHead(num_classes=num_classes, in_channels=in_channels)
    else:
        ...
        return MultiLinearClsHead(num_classes=num_classes,
...
class PoolingVitClassifier(BaseClassifier):
    ...
    def re_create_cl_head(self, ncls: int, in_dim: int = 768):
        self.classification_head = build_head(num_classes=ncls,
                                              dense_dims=self.dense_dims,
                                              in_channels=in_dim
                                              )

    def create_compound_cl_head(self, ncls_c: int,
                                dense_dims: str = 'None',
                                in_dim: int = 768):
        if dense_dims is None:  # use identical setup as the basic head.
            return build_head(num_classes=ncls_c,
                              dense_dims=self.dense_dims,
                              in_channels=in_dim
                              )
        else:
    ...
    def forward(self,
        ...
        # f: bsz, 768
        return f  # return features instead. use: self.classification_head(f)
        # for classification.
...
def run_poolingvitclassifier():
    ...
    model.eval()
    compound = model.create_compound_cl_head(11, '512-256-128')
    compound.eval()
    print("Num. parameters COMPOUND: {}".format(
        sum([p.numel() for p in compound.parameters()])))
    ...
    x = x.to(DEVICE)
    ft = model(x)
    logits = model.classification_head(ft)
    print(ft.shape, logits.shape, c, x.shape)
    print(compound(ft).shape)
